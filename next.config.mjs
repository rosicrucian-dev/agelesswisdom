...
// The plugin list lives in mdx.config.mjs so the search index generator can
// build the IDENTICAL pipeline — it reads back the paragraph anchors this
// pipeline assigns, and a drift between the two would deep-link readers to the
// wrong paragraph. See the header there.
import { rehypePlugins, remarkPlugins } from "./mdx.config.mjs";

const withMDX = createMDX({
  extension: /\.mdx?$/,
  // Plugin names stay strings here so the options are serializable for
  // Turbopack.

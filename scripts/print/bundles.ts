/**
 * Builds the "download everything" artifacts served from the About page:
 ...
 *   public/downloads/<section-id>.pdf — the same merged treatment per
 *                                        ADDITIONAL-material section (each is a
 *                                        separately-published work, so each
 *                                        reads as its own book rather than being
 *                                        folded into the curriculum volume).
 *
...
/** A minimal cover: the given name centered on an otherwise blank US-Letter
...
function buildMerged(groups: Grouped[], dest: string, coverTitle: string) {
  ...
  try {
    ...
    execFileSync("gs", [
      ...
      // Convert to sRGB rather than carrying Chrome's ICCBased spaces through.
      // Copying them, pdfwrite emits the profile as a ZERO-LENGTH stream: a
      // silently broken colour space that poppler and PDFium quietly fall back
      // from, but that CoreGraphics rejects — so Safari, Preview and anything
      // else on macOS drew every LESSON IMAGE AS BLANK in the merged bundles
      // while the per-lesson PDFs (never touched by gs) were fine. Emitting a
      // real sRGB profile is what makes the images survive the merge.
      "-dColorConversionStrategy=/sRGB",
      // Keep the scanned diagrams lossless. /prepress otherwise auto-selects
      // DCT for them, and JPEG ringing is plainly visible on hand-drawn line
      // art — a re-encoded page measures ~15% more dark pixels than the source.
...
function main() {
  ...
  // Zip carries everything (core + additional, separated); the curriculum's
  // merged PDF is School of Ageless Wisdom material only, and each additional
